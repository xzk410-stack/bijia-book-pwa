(() => {
  ...
  function rows(type){
    ...
    if(type==='category') list=[...ps().map(p=>p.category),...defaults.category];
    if(type==='store') list=[...ps().flatMap(p=>(p.records||[]).map(r=>r.store)),...defaults.store];
    const map=new Map();
    list.map(tidy).filter(Boolean).forEach(v=>{const k=key(v);if(!map.has(k))map.set(k,v)});
    return [...map.values()];
  }
  ...
  function install(id,type){
    ...
    const render=()=>{
      ...
      const items=rows(type).filter(v=>!q||key(v).includes(q)).sort((a,b)=>{const as=key(a).startsWith(q)?0:1,bs=key(b).startsWith(q)?0:1;return as-bs}).slice(0,5);
      if(!items.length){box.classList.remove('show');box.innerHTML='';return;}
      box.innerHTML=items.map(v=>'<button type="button"></button>').join('');
      [...box.children].forEach((b,i)=>{b.textContent=items[i];b.onmousedown=e=>e.preventDefault();b.onclick=()=>{input.value=items[i];box.classList.remove('show');input.dispatchEvent(new Event('change',{bubbles:true}))}});
