...
async function syncLocalToCloud(local){
  ...
  for(const p of bundle.payments){if(!paymentsByRecord.has(p.record_id))paymentsByRecord.set(p.record_id,[]);paymentsByRecord.get(p.record_id).push(p)}
  ...
  for(let i=0;i<local.length;i++){
    ...
    const wanted=Math.max(0,n(r.paid));if(wanted>existingTotal+0.005){const delta=Number((wanted-existingTotal).toFixed(2));const {error}=await db.from('spend_payments').insert({record_id:dbRow.id,user_id:session.user.id,amount:delta,method:r.paymethod||'舊資料匯入',paid_on:new Date().toISOString().slice(0,10)});if(error)throw error}
  }
  saveLocal(local);const latest=await fetchCloudBundle();dirty=false;localStorage.setItem('spend_cloud_seen_'+session.user.id,latest.newest||new Date().toISOString());localStorage.setItem('spend_migrated_'+session.user.id,'1');setSync('雲端已同步');
}
async function initialSync(){if(initializing||!session)return;initializing=true;try{setSync('同步中…');const local=getLocal();const cloud=await fetchCloudBundle();const migrated=localStorage.getItem('spend_migrated_'+session.user.id)==='1';if(!migrated&&local.length){const merged=mergePreferSecond(local,cloud.records);saveLocal(merged);await syncLocalToCloud(merged)}else if(cloud.records.length){saveLocal(cloud.records);localStorage.setItem('spend_cloud_seen_'+session.user.id,cloud.newest||new Date().toISOString());localStorage.setItem('spend_migrated_'+session.user.id,'1');setSync('雲端已同步')}else if(local.length){await syncLocalToCloud(local)}else{saveLocal([]);localStorage.setItem('spend_migrated_'+session.user.id,'1');setSync('雲端已同步')}showApp(true)}catch(e){console.error(e);setSync('同步失敗');setMessage('雲端同步暫時失敗，本機資料沒有被刪除。');showApp(true)}finally{initializing=false}}
function showApp(ok){const boot=el('boot');if(boot)boot.hidden=true;el('auth').classList.toggle('hidden',ok);el('topbar').classList.toggle('show',ok);el('appFrame').hidden=!ok;if(ok){el('accountEmail').textContent=session?.user?.email||'';try{el('appFrame').contentWindow.location.reload()}catch{}}}
