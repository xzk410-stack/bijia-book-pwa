...
function mergeRecords(a,b){const out=[],seen=new Set();for(const r of [...a,...b]){const k=fingerprint(r);if(seen.has(k))continue;seen.add(k);out.push(r)}return out.sort((x,y)=>(n(y.createdAt))-(n(x.createdAt)))}
function paymentFingerprint(p){return [n(p.amount),p.method||'',p.date||''].join('|')}
...
async function syncLocalToCloud(local){
  ...
  const paymentByRecord=new Map();
  for(const p of bundle.payments){if(!paymentByRecord.has(p.record_id))paymentByRecord.set(p.record_id,new Set());paymentByRecord.get(p.record_id).add(paymentFingerprint({amount:p.amount,method:p.method,date:p.paid_on}))}
  ...
  for(let i=0;i<local.length;i++){
    ...
    const common={name:r.name||'未命名商品',seller:r.seller||'未填寫',platform:r.platform||'其他',total_amount:Math.max(0,n(r.total)),paid_amount:Math.max(0,n(r.paid)),logistics_status:r.status||'尚未出貨',ship_date:r.shipdate||null,payment_method:r.paymethod||'',note:r.note||''};
    if(dbRow){
      const {data,error}=await db.from('spend_records').update(common).eq('id',dbRow.id).select('*').single();
      if(error)throw error;dbRow=data;
    }else{
      const insert={...common,user_id:session.user.id,base_amount:Math.max(0,n(r.total)),second_amount:0,fee_amount:0,shipping_amount:0,discount_amount:0,created_at:new Date(created).toISOString()};
    ...
    const existing=paymentByRecord.get(dbRow.id)||new Set();
    for(const p of Array.isArray(r.payments)?r.payments:[]){
      const fp=paymentFingerprint(p);if(existing.has(fp)||n(p.amount)<=0)continue;
      const {error}=await db.from('spend_payments').insert({record_id:dbRow.id,user_id:session.user.id,amount:n(p.amount),method:p.method||'',paid_on:p.date||new Date(created).toISOString().slice(0,10)});
      if(error)throw error;existing.add(fp)
    }
    paymentByRecord.set(dbRow.id,existing)
  }
  saveLocal(local);
...
async function initialSync(){
  ...
  try{
    ...
    if(!migrated&&local.length){const merged=mergeRecords(local,cloud.records);saveLocal(merged);await syncLocalToCloud(merged)}
